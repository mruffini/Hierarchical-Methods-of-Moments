...
# Add random noise
M = M + np.random.random(M.shape)/10
# Project the centers on the simplex
M = M/M.sum(0)

d,k = M.shape
# Random data generation
X, M, omega, x= rng.generate_sample_single_topic_model(n, d, k, c = 200, M = M)
...
CLSVD = eg.MAP_assign_clusters_stm(X, M, P)
tsvd =   time.time() - t0
print('SVD Time', tsvd)

pl.figure()
pl.imshow(X[CLSVD.argsort()].T, cmap='gray')
print('SVD Score', adjusted_rand_score(x,CLSVD))



# Run ALS method, calculates time and adjusted_rand_score
t0 = time.time()
M1,M2,M3 = eg.retrieve_tensors_stm(X)
M,P = om.learn_ALS(M1,M2,M3,k)
CLALS = eg.MAP_assign_clusters_stm(X, M, P)
tALS =   time.time() - t0
print('ALS Time', tALS)

pl.figure()
pl.imshow(X[CLALS.argsort()].T, cmap='gray')
print('ALS Score', adjusted_rand_score(x,CLALS))
